#!/usr/bin/env python3
"""
Simple Audio Test - Reliable Windows Audio Playback
==================================================
Simplified voice testing with reliable audio playback for Windows:
1. Saves audio files with proper names
2. Uses Windows built-in audio playback
3. Provides manual playback options
4. Works independently without complex dependencies

Usage:
    python simple_audio_test.py
    python simple_audio_test.py --customer priya
"""
...
class SimpleAudioTest:
    """
    Simple audio testing with reliable Windows playback
    """
    
    def __init__(self):
        """Initialize simple audio test system"""
        ...
        print("🎙️ Simple Audio Test System Initialized")
        print("🔊 Audio files will be saved and opened automatically")
        print("📁 Audio files location: voice_test_audio/")
    ...
    def save_and_play_audio(self, audio_bytes: bytes, audio_type: str, turn: int = None) -> str:
        """Save audio file and attempt to play it"""
        try:
            # Create descriptive filename
            if turn:
                filename = f"turn_{turn:02d}_{audio_type}_{int(time.time())}.wav"
            else:
                filename = f"{audio_type}_{int(time.time())}.wav"
            ...
            # Try to open with default Windows audio player
            try:
                os.startfile(str(audio_path))
                print(f"🔊 Opening {audio_type} with default audio player...")
                print("   (Audio should start playing automatically)")
                
                # Give user time to hear the audio
                if audio_type == "ai_response":
                    input("   Press ENTER after listening to the AI response...")
                else:
                    time.sleep(2)  # Brief pause for customer audio
    ...
    async def process_voice_interaction(self, text_input: str, customer_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            print("🔊 Step 2: Playing customer speech simulation...")
            customer_audio_path = self.save_and_play_audio(customer_audio, "customer_speech", self.turn_counter)
            
            # Step 3: Process customer audio through STT
            print("📝 Step 3: Converting speech back to text (STT)...")
            ...
            print("🤖 Step 4: Processing with Claude LLM...")
            ...
            print("🔊 Step 5: Converting AI response to speech...")
            ai_audio = await self.sarvam_handler.synthesize_tts(ai_response, language)
            
            # Save and play AI audio
            print("🎧 Step 6: Playing AI Agent Response...")
            ai_audio_path = self.save_and_play_audio(ai_audio, "ai_response", self.turn_counter)
            
            print(f"\n🤖 AI Agent Response: '{ai_response}'")
            print("✅ Voice interaction completed!")
            print(f"📁 Audio files saved for turn {self.turn_counter}")
...
IMPORTANT: Always respond in a helpful, empathetic manner focusing on finding solutions for the customer."""
    ...
    async def run_interactive_conversation(self, customer_key: str = "vijay"):
        """Run interactive conversation with simple audio playback"""
        if customer_key not in self.test_customers:
            print(f"❌ Customer '{customer_key}' not found")
            return
        
        customer_data = self.test_customers[customer_key]
        print(f"🎬 Starting conversation with {customer_data['name']}")
        print("=" * 60)
        ...
        print("🔊 Audio files will open automatically in your default player!")
        print("Commands:")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Simple Audio Test")
    parser.add_argument("--customer", type=str, choices=["vijay", "priya"],
                       default="vijay", help="Customer to test with")
    
    args = parser.parse_args()
    
    print("🎙️ Simple Audio Test System")
    print("=" * 60)
    print("🔊 Audio files will open automatically!")
    print("📁 Reliable Windows audio playback")
    print("📞 Independent call triggering")
    print("🎯 Complete voice pipeline testing")
    print("=" * 60)
    
    system = SimpleAudioTest()
    
    try:
        await system.run_interactive_conversation(args.customer)
